...
###!/cm/software/apps/python/2.7.13/bin/python
...
def read_system(filename):
    ...
    SystemDict = namedtuple('SystemDictionary', 'number, frag_type')
    #test= SystemDict._asdict
    #print 'test=',test
    #SystemDict = OrderedDict(['SystemDictionary','number', 'frag_type'])
    Tagpair = namedtuple('Tagpair', ['a', 'b']) #for flexible fragments
    #system_def = {}
    system_def = OrderedDict()
    #print system_def
    system_master = OrderedDict()
    system_tags = OrderedDict()
    ...
    for line in lines:
        if (line.strip().startswith('#')):
            ...
            if(start_reading_frags and not stop_reading_frags):
                    ...
                    #print "frag=",frag,"; number of=",number_of,"; frag_type=",frag_type.upper()[0]
                    system_def[frag] = SystemDict( int(number_of), frag_type.upper()[0])
		    # print "LOOOOOOOOKHERE3",system_def[frag]._asdict()
		    #print "LOOKHERE2",system_def
		  
    myfile.close()

    print("FIX THIS TO BE ORDERED! system_def=",system_def)

    #Now set up a dictionary with the actual atoms objects
    for species,info in system_def.items():
        print(species,info,"system def items",system_def.items())
        if info.frag_type == "A":
...
frag_list = []
tmp_sysdef = OrderedDict() #We can loop destructively over this
