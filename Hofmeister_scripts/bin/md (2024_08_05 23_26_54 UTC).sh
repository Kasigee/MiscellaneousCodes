...
if [ -z "\$finish" ]
   ...
   /cm/software/apps/dftbplus/19.1/bin/dftb+ >> \$run_dir/run.out
   date >> \$run_dir/run.out
fi
#
...
#   rm \$run_dir/run.out
#   date >> \$run_dir/run.out
#   echo "This dftb run is in run_dir \$run_dir and dir \$dir. Current directory is \$PWD. PBS work directory is \$PBS_O_WORKDIR" >> \$run_dir/run.out 
#   /cm/software/apps/dftbplus/19.1/bin/dftb+ >> \$run_dir/run.out
   date >> \$run_dir/run.out
   cd ..
