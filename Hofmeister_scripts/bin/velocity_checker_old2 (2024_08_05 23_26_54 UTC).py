...
#     print(test)
    ...
    for i in range(range_to_check):
        ...
        hist, bin_edges = np.histogram(calc_norms(vel), bins=50, density=False, range=(0,100))   
        # if first iteration no need to run calc_square_diff()
        if i == 0:
            hist_previous = hist
            continue

        sum_list.append(calc_square_diff(hist_previous, hist))
        hist2=hist_previous
        hist_previous = hist
...
'''
    ...
    for i in range(range_to_check):
        comment, vel = next(frame_velocities)
        timestep_norm=calc_norms(vel)
        NORM_diff=timestep_norm-timestep_history
        if i % 10:
            log.info(">> {}".format(comment))
        hist3, bin_edges = np.histogram(NORM_diff, bins=50, density=False, range=(0,100))
        # if first iteration no need to run calc_square_diff()
        if i == 0:
            timestep_history=timestep_norm
#            hist_previous = hist
            continue
        sum_list2.append(hist3)
 #       sum_list.append(calc_square_diff(hist_previous, hist))
  #      hist2=hist_previous
   #     hist_previous = hist
'''
    ...
    hist_ax = fig.add_subplot(1, 4, 1)
    vel_ax = fig.add_subplot(1, 4, 2)
    temp_ax = fig.add_subplot(1, 4, 3)
    toten_ax = fig.add_subplot(1, 4, 4)

    hist_ax.plot(hist, label="Actual Raw")
    hist_ax.plot(hist2, label="Actual Raw prior")
   # hist_ax.plot(sum_list2, label="Hist of diff values")
    ...
    vel_ax.legend()
    vel_ax.set_title("||Velocity|| distribution")
    vel_ax.set_xlabel("Frame # (not actual timestep)")
    vel_ax.set_ylabel("Speed (A/s I think)")
