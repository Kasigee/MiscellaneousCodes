...
Element1=$(cat $basefolder1/$molecule1/DDEC6_"$functional"_"$basis_set"/DDEC6_even_tempered_net_atomic_charges.xyz | awk '/The following XYZ/,/The spherically/' | head -n-2 | tail -n+3 | awk '{print $2}')
Element2=$(cat $basefolder2/$molecule2/DDEC6_"$functional"_"$basis_set"/DDEC6_even_tempered_net_atomic_charges.xyz | awk '/The following XYZ/,/The spherically/' | head -n-2 | tail -n+3 | awk '{print $2}')
r31=$(cat $basefolder1/$molecule1/DDEC6_"$functional"_"$basis_set"/DDEC_atomic_Rcubed_moments.xyz | tail -n$natoms1 | awk '{print $6^(1/3)}')
r32=$(cat $basefolder2/$molecule2/DDEC6_"$functional"_"$basis_set"/DDEC_atomic_Rcubed_moments.xyz | tail -n$natoms2 | awk '{print $6^(1/3)}')
...
if [ ! -f ../ion_int/$molecule1/$molecule2/"$molecule1""$molecule2"_SAPT_aug-cc-pvdz*.dat ]
then
